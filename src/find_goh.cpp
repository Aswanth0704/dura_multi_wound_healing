...
The apply the displacements at selected nodes in a small cicular region.
...
int main(int argc, char *argv[])
{
    ...
    double k0 = 0.00202026; //0.006375; // 0.0511; neo hookean for skin, used previously, in MPa
    double kf = 0.1833; //0.015; // stiffness of collagen in MPa, from previous paper

    if (argc >= 3) {
        k0 = std::stod(argv[1]);
        kf = std::stod(argv[2]);
    }   
    double k2 = 1.54; //0.048; // nonlinear exponential coefficient, non-dimensional
    double t_rho = 1e-1 * (1.28571E-5 / 55.05125); // 0.0045 force of fibroblasts in MPa
    double t_rho_c = 1e-1 * (1.28571E-5 * 3.28571 / 55.05125); // 0.045 force of myofibroblasts enhanced by chemical
    double K_t = 0.2; // Saturation of mechanical force by collagen
    double K_t_c = c_max / 10.; // saturation of chemical on force.
    double D_rhorho = 0.0833; // diffusion of cells in [mm^2/hour]
    double D_rhoc = 0.0; // diffusion of chemotactic gradient [mm^2/hour]
    double D_cc = 0.01208; // diffusion of chemical TGF, not normalized.
    double p_rho = 0.04958333 / 55.05126; // in 1/hour production of fibroblasts naturally
    double p_rho_c = 0.015314; // production enhanced by the chem
    double p_rho_theta = p_rho / 2; // enhanced production by theta
    double K_rho_c = c_max / 10.; // saturation of cell proliferation by chemical
    double K_rho_rho = 10000 * 55.05126; // saturation of cell by cell
    ...
    double p_c_thetaE = 300.0e-16 / rho_phys * 10; // coupling of elastic and chemical
    double K_c_c = 1.; // saturation of chem by chem
    double d_c = 0.01 / 2; // decay of chemical in 1/hours
    ...
    //
    // collagen fraction
    double p_phi = 0.002 / rho_phys;
    double p_phi_c = p_phi;
    double p_phi_theta = p_phi;
    double K_phi_c = 0.0001;
    double d_phi = 0.000970;
    double d_phi_rho_c = 0.5 * 0.000970 / rho_phys / c_max / 10;
    ...
    double rho_wound = 0.0;
    double c_wound = c_max;
    double phif0_wound = 1e-2;
    ...
    double rho_healthy = rho_phys;
    double c_healthy = 1e-4;
    double phif0_healthy = 1.;
    double kappa0_healthy = 0.111; //1. / 3;
    ...
    if (n0_wound(2) < 0) { n0_wound = a0_wound.cross(s0_wound); }
    Vector3d n0_healthy = s0_healthy.cross(a0_healthy);
    if (n0_healthy(2) < 0) { n0_healthy = a0_healthy.cross(s0_healthy); }
    //---------------------------------//
    ...
    std::string mesh_filename = "circular_wound_square_normal_finermesh.mphtxt";
    ...
    double x_center = 0.;
    double y_center = 0.;
    double x_axis = rad_load + tol_boundary;
    double y_axis = rad_load + tol_boundary;
    ...
    // ---- displacement list you requested ----
    const std::vector<double> z_displ_list = {2.63e-03, 3.18e-03, 4.17e-03, 8.67e-03, 1.10e-02};

    // output
    std::ofstream csv("force_vs_uz.csv");
    csv << "uz, force_z\n";

    // We will reuse the same patch node set across all runs
    std::vector<int> nodesZPush;

    for (double z_displ : z_displ_list)
    {
        // reset fields that must be reset each run (so displacements don't accumulate)
        ...
        // apply BCs (same logic as your original code)
        for (int nodei = 0; nodei < myTissue.n_node; nodei++)
        {
            ...
            if (myTissue.boundary_flag[nodei] == 1)
            {
                eBC_x.insert(std::pair<int,double>(nodei*3+0, myTissue.node_x[nodei](0)));
                eBC_x.insert(std::pair<int,double>(nodei*3+1, myTissue.node_x[nodei](1)));
                eBC_x.insert(std::pair<int,double>(nodei*3+2, myTissue.node_x[nodei](2)));
            }

            double check_ellipsoid =
                pow((x_coord - x_center), 2) / (x_axis * x_axis) +
                pow((y_coord - y_center), 2) / (y_axis * y_axis);

            if ((check_ellipsoid <= 1) && (z_coord >= (Zmax - tol_boundary)))
            {
                myTissue.node_x[nodei](2) = myTissue.node_x[nodei](2) - z_displ;
                nodesZPush.push_back(nodei);
                eBC_x.insert(std::pair<int,double>(nodei*3+2, myTissue.node_x[nodei](2)));
            }

            eBC_rho.insert(std::pair<int,double>(nodei, myTissue.node_rho[nodei]));
        ...
        myTissue.time_final = (7*24) + 1;
        myTissue.time_step = 0.2;
        myTissue.tol = 1e-8;
        myTissue.max_iter = 25;

        // IMPORTANT: if fillDOFmap/evalElemJacobians depend on BC maps, keep them inside the loop
        fillDOFmap(myTissue);
        ...
        std::stringstream ss;
        std::string filename = "circular_paraviewoutput_mech" + ss.str() + "_";

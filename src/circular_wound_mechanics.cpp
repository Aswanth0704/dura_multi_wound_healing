...
int main(int argc, char *argv[])
{
	...
	double rad = 0.282;
    double rad_load = 0.05; // need to provide approximate value of the point load and also make sure that there are elements inside this.
	double h = 0.1;
	double g = 0.1;
	double Xmin = -rad -g;
	double Xmax = rad + g;
	...
	double z_displ = 0.005; // included here so that I can run a loop over all the stretches
    std::vector<int> nodesZPush; // actually I might need elements. but save this anyway
    std::vector<int> elemsZPush;
    std::vector<double> elemsZPushArea; // area of the elements that are pushed

	// let me first fixt the boundary conditions for rho and c.
	// Here all the nodes will be fixed with rho and c values

	for (int nodei=0;nodei<myTissue.n_node;nodei++){

		// Next I will fix all the displacements at the boundary nodes.
		double x_coord = myTissue.node_x[nodei](0);
		double y_coord = myTissue.node_x[nodei](1);
		double z_coord = myTissue.node_x[nodei](2);


		// check if the node is on the boundary. (east, west, north, south)
		...
		// check if the node is in the loaded region. Here, I will apply the displacement to the z coordinate and fix it
		// Only thing is that i have applied the z-displacement across the thickness.
	...
	// 
	std::cout << "Loaded patch contains "<<nodesZPush.size()<<" nodes"<<std::endl;
	...
	// I will read the list of all the displacements.

...
int main(int argc, char *argv[])
{
    ...
    std::string mesh_filename = "dura_only_double_wound_2t.msh";
    HexMesh myMesh = readGmshMsh22doubleWound(
        mesh_filename,
        "bottom_end_nodes_double_wound_2t.txt",
        "top_end_nodes_double_wound_2t.txt",
        101, 104, 102
    );
    ...
    // std::cout<<"elements domain ids \n";
    // for(int elemi=0;elemi<myMesh.n_elements;elemi++){
    //     int id = myMesh.element_domain_id[elemi];
    //     if (id == 101){
    //         std::cout<<"wound_one"<<std::endl;
    //     }
    //     else if(id == 104){
    //         std::cout<<"wound_two"<<std::endl;
    //     }
    //     else if (id == 102){
    //         std::cout<<"healthy"<<std::endl;
    //     }
    //     else{
    //         std::cout<<"incorrect domain"<<std::endl;
    //     }
    ...
    // I have domain id for each element, 102 for healthy and 101 for the 1st wound and 104 for the sec wound.
    // how do I decide if the node belongs to the healthy or wound1 or wound2.
    ...
    // 101 = wound1,104 = wound2, 102 = healthy
    std::vector<unsigned char> node_is_wound1(myMesh.n_nodes, 0);
    std::vector<unsigned char> node_is_wound2(myMesh.n_nodes, 0);      
    std::vector<unsigned char> node_is_healthy(myMesh.n_nodes, 0);    
    ...
    for(int elemi = 0; elemi < myMesh.n_elements; ++elemi)
    {
        ...
        if(dom == 101) {
            for(int nid : myMesh.elements[elemi]) node_is_wound1[nid] = 1;
        } else if (dom == 104){
            for(int nid : myMesh.elements[elemi]) node_is_wound2[nid] = 1;
        }
         else if(dom == 102) {
    ...
    std::vector<unsigned char> node_is_interface1(myMesh.n_nodes, 0);
    for(int i=0; i<myMesh.n_nodes; ++i){
        if(node_is_wound1[i] && node_is_healthy[i]) node_is_interface1[i] = 1;
    }
	...
	for(int nodei=0;nodei<myMesh.n_nodes;nodei++){
        ...
        if(node_is_wound1[nodei]){   // Option A: wound wins
	...
	myTissue.ip_lamdaP = ip_lamda0; // why?
    myTissue.ip_lamdaE = ip_lamda0; // why?
	...
	std::stringstream ss;
	std::string filename = "exact_dura_double_wound_1_2t"+ss.str()+"_";
    ...
    // Inflict second wound:
    for(int nodei=0;nodei<myMesh.n_nodes;nodei++){
		
        if(node_is_wound2[nodei]){   // Option A: wound wins
            std::cout << "wound node 2 " << nodei << "\n";
	...
	for(int elemi=0; elemi<myMesh.n_elements; ++elemi){
        std::cout << "wound elem 2 " << elemi << "\n";
        bool is_wound_elem = (myMesh.element_domain_id[elemi] == 104);

        if(is_wound_elem){
            for(int ip=0; ip<IP_size; ++ip){
                myTissue.ip_phif_0[elemi*IP_size+ip] = phif0_wound;	
				myTissue.ip_phif[elemi*IP_size+ip] = phif0_wound;	
				myTissue.ip_a0_0[elemi*IP_size+ip] = a0_wound;
				myTissue.ip_a0[elemi*IP_size+ip] = a0_wound;
				myTissue.ip_s0_0[elemi*IP_size+ip] = s0_wound;
				myTissue.ip_s0[elemi*IP_size+ip] = s0_wound;
				myTissue.ip_n0_0[elemi*IP_size+ip] = n0_wound;
				myTissue.ip_n0[elemi*IP_size+ip] = n0_wound;
    ...
    // filename_step  = "exact_dura_wound_2_output_" + tag + ".vtk";
    // filename_step2 = "exact_dura_wound_2_output_second_" + tag + ".vtk";
	...
	std::string filename2 = "exact_dura_double_wound_2_2t"+ss2.str()+"_";
